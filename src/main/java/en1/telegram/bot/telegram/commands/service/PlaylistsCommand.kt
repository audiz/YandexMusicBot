 ...
 * All playlists
 */
class PlaylistsCommand(identifier: String?, description: String?, private val musicService: CallbackMusicActions,
                       private val messageSender: MessageSender) : BotCommand(identifier, description) {
    private val logger = LoggerFactory.getLogger(PlaylistsCommand::class.java)
    ...
    override fun execute(absSender: AbsSender, user: User, chat: Chat, strings: Array<String>) {
        ...
        val callbackResult = musicService.getPlaylists(chat.id.toString())
